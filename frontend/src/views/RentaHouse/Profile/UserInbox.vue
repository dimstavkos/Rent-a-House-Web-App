...
const headers = ["ID", "Sent By", "Date", "Text", "Action"];
...
const getMessages = () => {
  let id = store.id;
  const api = "http://localhost:8000/usermessage/inbox/1" ;
...
<template>
  <BaseLayout
    title="Inbox"
    :breadcrumb="[{ label: 'User' }, { label: 'Inbox' }]"
  >
        ...
        <tbody>
          <tr v-for="u in filteredDataset" :key="u.id">
            ...
            <td class="align-middle text-center">
              ...
              <RouterLink :to="{ name: 'user-compose', params: { id: u.id } }" rel="tooltip" title="Reply"
                data-placement="bottom">
                <i class="material-icons me-2 cursor-pointer" aria-hidden="true">reply</i>
              </RouterLink>

              <RouterLink :to="{ name: 'user-compose', params: { id: u.id } }" rel="tooltip" title="Delete"
                data-placement="bottom">
                <i class="material-icons me-2 cursor-pointer" aria-hidden="true">delete</i>
              </RouterLink>
